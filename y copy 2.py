...
def assign_roles(topic):
    conversationalist = Agent(
        ...
        goal=f"Gradually introduce the topic of {topic}, facilitate an engaging discussion like Lex Fridman or Joe Rogan",
...
async def simulate_podcast(duration_minutes=30):
    ...
    while asyncio.get_event_loop().time() < end_time:
        ...
        conversationalist_task = Task(
            description=f"Continue the discussion on {topic}, gradually increasing in depth. Ask thought-provoking questions or provide interesting perspectives. If applicable, respond to the Expert's previous comments. Current conversation: {conversation}",
            agent=conversationalist,
...
def handle_listener_interaction(conversation, current_speaker):
    ...
    if listener_input.lower() == 'y':
        ...
        crew = Crew(agents=[current_speaker], tasks=[answer_task])
        answer = crew.kickoff()
        conversation.append(f"{current_speaker.role}: {answer}")
        print(f"{current_speaker.role}: {answer}")
        return True
    return False
